...
export function AboutSection() {
  ...
  return (
    <section>
      <h2 style={{ margin: '0 0 16px', fontSize: 18 }}>{t('about.title')}</h2>
      <dl style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '8px 24px', margin: 0 }}>
        {rows.map(([label, value]) => (
          <div key={label} style={{ display: 'contents' }}>
            <dt style={{ color: token('textMuted'), fontSize: 13 }}>{label}</dt>
            <dd style={{ margin: 0, fontSize: 13, fontFamily: 'ui-monospace, monospace' }}>
